...
export const EXPEL_NAME = 'expel';
const INITIAL_URL =
    'https://boards-api.greenhouse.io/v1/boards/expel/jobs?content=true';
...
const scrapJobPost = async ({
    ...
    try {
        ...
        const error = errorWithPrefix(
            e,
            `Error processing ${EXPEL_NAME} job post ${id}`,
        );
...
export const expelScrapper: NewCompanyScrapper = ({ companyId }) => {
    return {
        getListedJobPostsData: async () => {
            ...
            jobsData.jobs.forEach((jobData) => {
                ...
                if (
                    !title.includes('join our community') &&

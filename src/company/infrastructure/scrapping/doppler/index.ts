...
export const DOPPLER_NAME = 'doppler';
const DOPPLER_INITIAL_URL =
    'https://api.ashbyhq.com/posting-api/job-board/doppler';
...
const scrapJobPost = async ({
    ...
    try {
        ...
        const error = errorWithPrefix(
            e,
            `Error processing ${DOPPLER_NAME} job post ${id}`,
        );
...
export const dopplerScrapper: NewCompanyScrapper = ({ companyId }) => {
    return {
        getListedJobPostsData: async () => {
            const response = await fetch(DOPPLER_INITIAL_URL);

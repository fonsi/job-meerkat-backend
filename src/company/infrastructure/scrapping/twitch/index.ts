...
export const TWITCH_NAME = 'twitch';
const TWITCH_INITIAL_URL =
    'https://boards-api.greenhouse.io/v1/boards/twitch/jobs?content=true';
...
const scrapJobPost = async ({
    ...
    try {
        ...
        const error = errorWithPrefix(
            e,
            `Error processing ${TWITCH_NAME} job post ${id}`,
        );
...
export const twitchScrapper: NewCompanyScrapper = ({ companyId }) => {
    return {
        getListedJobPostsData: async () => {
            const response = await fetch(TWITCH_INITIAL_URL);

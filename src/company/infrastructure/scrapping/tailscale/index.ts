...
export const TAILSCALE_NAME = 'tailscale';
const INITIAL_URL =
...
const scrapJobPost = async ({
    ...
    try {
        ...
        const error = errorWithPrefix(
            e,
            `Error processing ${TAILSCALE_NAME} job post ${id}`,
        );
...
export const tailscaleScrapper: NewCompanyScrapper = ({ companyId }) => {
    return {
        ...
        scrapJobPost: async (jobPosts: ListedJobPostsData[]) => {
            ...
            for (let i = 0; i < jobPosts.length; i++) {
                try {
                    ...
                    const error = errorWithPrefix(
                        e,
                        `[Error processing ${TAILSCALE_NAME}]`,
                    );

...
export const WISPR_FLOW_NAME = 'wispr flow';
const WISPR_FLOW_NAME_INITIAL_URL =
    'https://api.ashbyhq.com/posting-api/job-board/wispr-flow';
...
const scrapJobPost = async ({
    ...
    try {
        ...
        const error = errorWithPrefix(
            e,
            `Error processing ${WISPR_FLOW_NAME} job post ${id}`,
        );
...
export const wisprFlowScrapper: NewCompanyScrapper = ({ companyId }) => {
    return {
        getListedJobPostsData: async () => {
            const response = await fetch(WISPR_FLOW_NAME_INITIAL_URL);
        ...
        scrapJobPost: async (jobPosts: ListedJobPostsData[]) => {
            ...
            for (let i = 0; i < jobPosts.length; i++) {
                try {
                    ...
                    const error = errorWithPrefix(
                        e,
                        `[Error processing ${WISPR_FLOW_NAME}]`,
                    );

...
export const INCIDENT_IO_NAME = 'incident.io';
const INITIAL_URL = 'https://api.ashbyhq.com/posting-api/job-board/incident';
...
const scrapJobPost = async ({
    ...
    try {
        ...
        const error = errorWithPrefix(
            e,
            `Error processing ${INCIDENT_IO_NAME} job post ${id}`,
        );
...
export const incidentIoScrapper: NewCompanyScrapper = ({ companyId }) => {
    return {
        ...
        scrapJobPost: async (jobPosts: ListedJobPostsData[]) => {
            ...
            for (let i = 0; i < jobPosts.length; i++) {
                try {
                    ...
                    const error = errorWithPrefix(
                        e,
                        `[Error processing ${INCIDENT_IO_NAME}]`,
                    );

...
export const FREEWILL_NAME = 'freewill';
const FREEWILL_NAME_INITIAL_URL =
    'https://api.ashbyhq.com/posting-api/job-board/freewill';
...
const scrapJobPost = async ({
    ...
    try {
        ...
        const error = errorWithPrefix(
            e,
            `Error processing ${FREEWILL_NAME} job post ${id}`,
        );
...
export const freewillScrapper: NewCompanyScrapper = ({ companyId }) => {
    return {
        getListedJobPostsData: async () => {
            const response = await fetch(FREEWILL_NAME_INITIAL_URL);
        ...
        scrapJobPost: async (jobPosts: ListedJobPostsData[]) => {
            ...
            for (let i = 0; i < jobPosts.length; i++) {
                try {
                    ...
                    const error = errorWithPrefix(
                        e,
                        `[Error processing ${FREEWILL_NAME}]`,
                    );

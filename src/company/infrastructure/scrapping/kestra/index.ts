...
export const KESTRA_NAME = 'kestra';
const KESTRA_INITIAL_URL =
    'https://api.ashbyhq.com/posting-api/job-board/kestra';
...
const scrapJobPost = async ({
    ...
    try {
        ...
        const error = errorWithPrefix(
            e,
            `Error processing ${KESTRA_NAME} job post ${id}`,
        );
...
export const kestraScrapper: NewCompanyScrapper = ({ companyId }) => {
    return {
        getListedJobPostsData: async () => {
            const response = await fetch(KESTRA_INITIAL_URL);

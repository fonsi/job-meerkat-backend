...
const INITIAL_URL = 'https://api.ashbyhq.com/posting-api/job-board/render';
...
const scrapJobPost = async ({
    ...
    try {
        ...
        const error = errorWithPrefix(
            e,
            `Error processing ${RENDER_NAME} job post ${id}`,
        );
...
export const renderScrapper: NewCompanyScrapper = ({ companyId }) => {
    return {
        getListedJobPostsData: async () => {
            ...
            jobsData.jobs.forEach((jobData) => {
                ...
                if (jobData.isListed && !title.includes('open application')) {
                    jobPosts.push({
                        id: jobData.id,
                        url: jobData.jobUrl,
                        title: jobData.title,

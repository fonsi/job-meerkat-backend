...
export const AIRTABLE_NAME = 'airtable';
const INITIAL_URL =
    'https://boards-api.greenhouse.io/v1/boards/airtable/jobs?content=true';
...
const scrapJobPost = async ({
    ...
    try {
        ...
        const error = errorWithPrefix(
            e,
            `Error processing ${AIRTABLE_NAME} job post ${id}`,
        );
...
export const airtableScrapper: NewCompanyScrapper = ({ companyId }) => {
    return {
        ...
        scrapJobPost: async (jobPosts: ListedJobPostsData[]) => {
            ...
            for (let i = 0; i < jobPosts.length; i++) {
                try {
                    ...
                    const error = errorWithPrefix(
                        e,
                        `[Error processing ${AIRTABLE_NAME}]`,
                    );

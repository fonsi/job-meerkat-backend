...
export const POSTSCRIPT_NAME = 'postscript';
const POSTSCRIPT_INITIAL_URL = 'https://job-boards.greenhouse.io/postscript';
...
const scrapJobPost = async ({
    ...
    try {
        ...
        const error = errorWithPrefix(
            e,
            `Error processing ${POSTSCRIPT_NAME} job post ${id}`,
        );
...
export const postscriptScrapper: CompanyScrapperFn = async ({ companyId }) => {
    const $ = await fromURL(POSTSCRIPT_INITIAL_URL);
    ...
    for (let i = 0; i < jobPosts.length; i++) {
        try {
            ...
            const error = errorWithPrefix(
                e,
                `[Error processing ${POSTSCRIPT_NAME}]`,
            );

...
export const GITLAB_NAME = 'gitlab';
const GITLAB_NAME_INITIAL_URL =
...
const scrapJobPost = async ({
    ...
    try {
        ...
        const error = errorWithPrefix(
            e,
            `Error processing ${GITLAB_NAME} job post ${id}`,
        );
...
export const gitlabScrapper: NewCompanyScrapper = ({ companyId }) => {
    return {
        getListedJobPostsData: async () => {
            const response = await fetch(GITLAB_NAME_INITIAL_URL);

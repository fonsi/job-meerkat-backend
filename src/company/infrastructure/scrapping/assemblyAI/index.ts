...
export const ASSEMBLY_AI_NAME = 'assemblyai';
const ASSEMBLY_AI_INITIAL_URL = 'https://job-boards.greenhouse.io/assemblyai';
...
const scrapJobPost = async ({
    ...
    try {
        ...
        const error = errorWithPrefix(
            e,
            `Error processing ${ASSEMBLY_AI_NAME} job post ${id}`,
        );
...
export const assemblyAiScrapper: CompanyScrapperFn = async ({ companyId }) => {
    const $ = await fromURL(ASSEMBLY_AI_INITIAL_URL);
    ...
    for (let i = 0; i < jobPosts.length; i++) {
        try {
            ...
            const error = errorWithPrefix(
                e,
                `[Error processing ${ASSEMBLY_AI_NAME}]`,
            );

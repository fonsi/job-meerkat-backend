...
export const HONEYCOMB_NAME = 'honeycomb';
const INITIAL_URL =
    'https://boards-api.greenhouse.io/v1/boards/honeycomb/jobs?content=true';
...
const scrapJobPost = async ({
    ...
    try {
        ...
        const error = errorWithPrefix(
            e,
            `Error processing ${HONEYCOMB_NAME} job post ${id}`,
        );
...
export const honeycombScrapper: NewCompanyScrapper = ({ companyId }) => {
    return {
        ...
        scrapJobPost: async (jobPosts: ListedJobPostsData[]) => {
            ...
            for (let i = 0; i < jobPosts.length; i++) {
                try {
                    ...
                    const error = errorWithPrefix(
                        e,
                        `[Error processing ${HONEYCOMB_NAME}]`,
                    );

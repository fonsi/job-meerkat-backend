...
export const HELIUS_NAME = 'helius';
const HELIUS_INITIAL_URL =
    'https://api.ashbyhq.com/posting-api/job-board/helius';
...
const scrapJobPost = async ({
    ...
    try {
        ...
        const error = errorWithPrefix(
            e,
            `Error processing ${HELIUS_NAME} job post ${id}`,
        );
...
export const heliusScrapper: NewCompanyScrapper = ({ companyId }) => {
    return {
        getListedJobPostsData: async () => {
            const response = await fetch(HELIUS_INITIAL_URL);

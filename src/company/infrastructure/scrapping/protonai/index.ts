...
export const PROTONAI_NAME = 'proton.ai';
const PROTONAI_INITIAL_URL = 'https://job-boards.greenhouse.io/protonai';
...
const scrapJobPost = async ({
    ...
    try {
        ...
        const error = errorWithPrefix(
            e,
            `Error processing ${PROTONAI_NAME} job post ${id}`,
        );
...
export const protonaiScrapper: NewCompanyScrapper = ({ companyId }) => {
    return {
        getListedJobPostsData: async () => {
            const $ = await fromURL(PROTONAI_INITIAL_URL);
        ...
        scrapJobPost: async (jobPosts: ListedJobPostsData[]) => {
            ...
            for (let i = 0; i < jobPosts.length; i++) {
                try {
                    ...
                    const error = errorWithPrefix(
                        e,
                        `[Error processing ${PROTONAI_NAME}]`,
                    );

...
export const AIRBYTE_NAME = 'airbyte';
const AIRBYTE_INITIAL_URL =
...
const scrapJobPost = async ({
    ...
    try {
        ...
        const error = errorWithPrefix(
            e,
            `Error processing ${AIRBYTE_NAME} job post ${id}`,
        );
...
export const airbyteScrapper: NewCompanyScrapper = ({ companyId }) => {
    return {
        getListedJobPostsData: async () => {
            const response = await fetch(AIRBYTE_INITIAL_URL);

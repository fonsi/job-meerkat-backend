...
export const MAZE_HQ_NAME = 'maze hq';
const ASHBY_COMPANY_NAME = 'mazehq';
const MAZE_HQ_INITIAL_URL = `https://api.ashbyhq.com/posting-api/job-board/${ASHBY_COMPANY_NAME}`;
...
const scrapJobPost = async ({
    ...
    try {
        ...
        const error = errorWithPrefix(
            e,
            `Error processing ${MAZE_HQ_NAME} job post ${id}`,
        );
...
export const mazeHqScrapper: NewCompanyScrapper = ({ companyId }) => {
    return {
        getListedJobPostsData: async () => {
            const response = await fetch(MAZE_HQ_INITIAL_URL);

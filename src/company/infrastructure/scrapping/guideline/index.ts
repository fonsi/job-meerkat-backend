...
export const GUIDELINE_NAME = 'guideline';
const GUIDELINE_INITIAL_URL = 'https://job-boards.greenhouse.io/guideline';
...
const scrapJobPost = async ({
    ...
    try {
        ...
        const error = errorWithPrefix(
            e,
            `Error processing ${GUIDELINE_NAME} job post ${id}`,
        );
...
export const guidelineScrapper: CompanyScrapperFn = async ({ companyId }) => {
    const $ = await fromURL(GUIDELINE_INITIAL_URL);
    ...
    for (let i = 0; i < jobPosts.length; i++) {
        try {
            ...
            const error = errorWithPrefix(
                e,
                `[Error processing ${GUIDELINE_NAME}]`,
            );

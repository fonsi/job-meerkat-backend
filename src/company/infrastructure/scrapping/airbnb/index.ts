...
export const AIRBNB_NAME = 'airbnb';
const AIRBNB_INITIAL_URL =
...
const JOB_HEADER_SELECTOR = '.job-page-banner';
const JOB_CONTENT_SELECTOR = '.job-detail';
...
const scrapJobPost = async ({
    ...
    try {
        ...
        const error = errorWithPrefix(
            e,
            `Error processing ${AIRBNB_NAME} job post ${id}`,
        );
...
export const airbnbScrapper: NewCompanyScrapper = ({ companyId }) => {
    return {
        getListedJobPostsData: async () => {
            const response = await fetch(AIRBNB_INITIAL_URL);

...
export const TRIGGER_DEV_NAME = 'trigger.dev';
const ASHBY_COMPANY_NAME = 'triggerdev';
const TRIGGER_DEV_INITIAL_URL =
    'https://api.ashbyhq.com/posting-api/job-board/triggerdev';
...
const scrapJobPost = async ({
    ...
    try {
        ...
        const error = errorWithPrefix(
            e,
            `Error processing ${TRIGGER_DEV_NAME} job post ${id}`,
        );
...
export const triggerDevScrapper: NewCompanyScrapper = ({ companyId }) => {
    return {
        getListedJobPostsData: async () => {
            const response = await fetch(TRIGGER_DEV_INITIAL_URL);
        ...
        scrapJobPost: async (jobPosts: ListedJobPostsData[]) => {
            ...
            for (let i = 0; i < jobPosts.length; i++) {
                try {
                    ...
                    const error = errorWithPrefix(
                        e,
                        `Error processing ${TRIGGER_DEV_NAME}`,
                    );

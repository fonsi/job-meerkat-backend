...
export const MATERIALIZE_NAME = 'materialize';
const INITIAL_URL = 'https://api.ashbyhq.com/posting-api/job-board/materialize';
...
const scrapJobPost = async ({
    ...
    try {
        ...
        const error = errorWithPrefix(
            e,
            `Error processing ${MATERIALIZE_NAME} job post ${id}`,
        );
...
export const materializeScrapper: NewCompanyScrapper = ({ companyId }) => {
    return {
        ...
        scrapJobPost: async (jobPosts: ListedJobPostsData[]) => {
            ...
            for (let i = 0; i < jobPosts.length; i++) {
                try {
                    ...
                    const error = errorWithPrefix(
                        e,
                        `[Error processing ${MATERIALIZE_NAME}]`,
                    );

...
export const LINEAR_NAME = 'linear';
const LINEAR_INITIAL_URL =
...
const scrapJobPost = async ({
    ...
    try {
        ...
        const error = errorWithPrefix(
            e,
            `Error processing ${LINEAR_NAME} job post ${id}`,
        );
...
export const linearScrapper: NewCompanyScrapper = ({ companyId }) => {
    return {
        getListedJobPostsData: async () => {
            const response = await fetch(LINEAR_INITIAL_URL);

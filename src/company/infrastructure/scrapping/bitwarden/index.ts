...
export const BITWARDEN_NAME = 'bitwarden';
const INITIAL_URL =
    'https://boards-api.greenhouse.io/v1/boards/bitwarden/jobs?content=true';
...
const scrapJobPost = async ({
    ...
    try {
        ...
        const error = errorWithPrefix(
            e,
            `Error processing ${BITWARDEN_NAME} job post ${id}`,
        );
...
export const bitwardenScrapper: NewCompanyScrapper = ({ companyId }) => {
    return {
        ...
        scrapJobPost: async (jobPosts: ListedJobPostsData[]) => {
            ...
            for (let i = 0; i < jobPosts.length; i++) {
                try {
                    ...
                    const error = errorWithPrefix(
                        e,
                        `[Error processing ${BITWARDEN_NAME}]`,
                    );

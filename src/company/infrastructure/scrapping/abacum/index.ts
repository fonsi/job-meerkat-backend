...
export const ABACUM_NAME = 'abacum';
const ABACUM_INITIAL_URL =
    'https://api.ashbyhq.com/posting-api/job-board/abacum';
...
const scrapJobPost = async ({
    ...
    try {
        ...
        const error = errorWithPrefix(
            e,
            `Error processing ${ABACUM_NAME} job post ${id}`,
        );
...
export const abacumScrapper: NewCompanyScrapper = ({ companyId }) => {
    return {
        getListedJobPostsData: async () => {
            const response = await fetch(ABACUM_INITIAL_URL);

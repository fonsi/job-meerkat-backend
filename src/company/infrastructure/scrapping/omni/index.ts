...
export const OMNI_NAME = 'omni';
const OMNI_INITIAL_URL = 'https://api.ashbyhq.com/posting-api/job-board/omni';
...
const scrapJobPost = async ({
    ...
    try {
        ...
        const error = errorWithPrefix(
            e,
            `Error processing ${OMNI_NAME} job post ${id}`,
        );
...
export const omniScrapper: NewCompanyScrapper = ({ companyId }) => {
    return {
        getListedJobPostsData: async () => {
            const response = await fetch(OMNI_INITIAL_URL);

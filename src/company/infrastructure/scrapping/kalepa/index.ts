...
export const KALEPA_NAME = 'kalepa';
const KALEPA_INITIAL_URL =
    'https://boards-api.greenhouse.io/v1/boards/kalepa/jobs?content=true';
...
const scrapJobPost = async ({
    ...
    try {
        ...
        const error = errorWithPrefix(
            e,
            `Error processing ${KALEPA_NAME} job post ${id}`,
        );
...
export const kalepaScrapper: NewCompanyScrapper = ({ companyId }) => {
    return {
        getListedJobPostsData: async () => {
            const response = await fetch(KALEPA_INITIAL_URL);

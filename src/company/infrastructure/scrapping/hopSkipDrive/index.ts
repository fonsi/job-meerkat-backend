...
export const HOP_SKIP_DRIVE_NAME = 'hopskipdrive';
const HOP_SKIP_DRIVE_NAME_INITIAL_URL =
    'https://boards-api.greenhouse.io/v1/boards/hopskipdrive/jobs?content=true';
...
const scrapJobPost = async ({
    ...
    try {
        ...
        const error = errorWithPrefix(
            e,
            `Error processing ${HOP_SKIP_DRIVE_NAME} job post ${id}`,
        );
...
export const hopSkipDriveScrapper: NewCompanyScrapper = ({ companyId }) => {
    return {
        getListedJobPostsData: async () => {
            const response = await fetch(HOP_SKIP_DRIVE_NAME_INITIAL_URL);
        ...
        scrapJobPost: async (jobPosts: ListedJobPostsData[]) => {
            ...
            for (let i = 0; i < jobPosts.length; i++) {
                try {
                    ...
                    const error = errorWithPrefix(
                        e,
                        `[Error processing ${HOP_SKIP_DRIVE_NAME}]`,
                    );

...
export const FIGMA_NAME = 'figma';
const FIGMA_NAME_INITIAL_URL =
...
const scrapJobPost = async ({
    ...
    try {
        ...
        const error = errorWithPrefix(
            e,
            `Error processing ${FIGMA_NAME} job post ${id}`,
        );
...
export const figmaScrapper: NewCompanyScrapper = ({ companyId }) => {
    return {
        getListedJobPostsData: async () => {
            const response = await fetch(FIGMA_NAME_INITIAL_URL);

...
export const CAMUNDA_NAME = 'camunda';
const CAMUNDA_NAME_INITIAL_URL =
    'https://api.ashbyhq.com/posting-api/job-board/camunda';
...
const scrapJobPost = async ({
    ...
    try {
        ...
        const error = errorWithPrefix(
            e,
            `Error processing ${CAMUNDA_NAME} job post ${id}`,
        );
...
export const camundaScrapper: NewCompanyScrapper = ({ companyId }) => {
    return {
        getListedJobPostsData: async () => {
            const response = await fetch(CAMUNDA_NAME_INITIAL_URL);

...
const BUFFER_NAME_INITIAL_URL =
    'https://api.ashbyhq.com/posting-api/job-board/buffer';
...
const scrapJobPost = async ({
    ...
    try {
        ...
        const error = errorWithPrefix(
            e,
            `Error processing ${BUFFER_NAME} job post ${id}`,
        );
...
export const bufferScrapper: NewCompanyScrapper = ({ companyId }) => {
    return {
        getListedJobPostsData: async () => {
            const response = await fetch(BUFFER_NAME_INITIAL_URL);

...
export const GRAFANA_LABS_NAME = 'grafana labs';
const GRAFANA_LABS_NAME_INITIAL_URL =
...
const scrapJobPost = async ({
    ...
    try {
        ...
        const error = errorWithPrefix(
            e,
            `Error processing ${GRAFANA_LABS_NAME} job post ${id}`,
        );
...
export const grafanaLabsScrapper: NewCompanyScrapper = ({ companyId }) => {
    return {
        getListedJobPostsData: async () => {
            const response = await fetch(GRAFANA_LABS_NAME_INITIAL_URL);
            const jobsData = await response.json();

            const jobPosts: JobPostsListItem[] = jobsData.jobs
                .map((jobData) => ({
                ...
                .filter(({ title }) => {
                    ...
                    return (
                        ...
                        !lower.includes('future sales openings')
                    );
                });
        ...
        scrapJobPost: async (jobPosts: ListedJobPostsData[]) => {
            ...
            for (let i = 0; i < jobPosts.length; i++) {
                try {
                    ...
                    const error = errorWithPrefix(
                        e,
                        `Error processing ${GRAFANA_LABS_NAME}`,
                    );

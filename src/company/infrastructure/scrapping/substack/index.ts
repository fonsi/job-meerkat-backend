...
export const SUBSTACK_NAME = 'substack';
const INITIAL_URL = 'https://api.ashbyhq.com/posting-api/job-board/substack';
...
const scrapJobPost = async ({
    ...
    try {
        ...
        const error = errorWithPrefix(
            e,
            `Error processing ${SUBSTACK_NAME} job post ${id}`,
        );
...
export const substackScrapper: NewCompanyScrapper = ({ companyId }) => {
    return {
        ...
        scrapJobPost: async (jobPosts: ListedJobPostsData[]) => {
            ...
            for (let i = 0; i < jobPosts.length; i++) {
                try {
                    ...
                    const error = errorWithPrefix(
                        e,
                        `[Error processing ${SUBSTACK_NAME}]`,
                    );

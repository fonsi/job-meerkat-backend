...
const RIVER_NAME_INITIAL_URL =
    'https://api.ashbyhq.com/posting-api/job-board/river';
...
const scrapJobPost = async ({
    ...
    try {
        ...
        const error = errorWithPrefix(
            e,
            `Error processing ${RIVER_NAME} job post ${id}`,
        );
...
export const riverScrapper: NewCompanyScrapper = ({ companyId }) => {
    return {
        getListedJobPostsData: async () => {
            const response = await fetch(RIVER_NAME_INITIAL_URL);

...
const REVENUE_CAT_INITIAL_URL =
    'https://api.ashbyhq.com/posting-api/job-board/revenuecat';
...
const scrapJobPost = async ({
    ...
    try {
        ...
        const error = errorWithPrefix(
            e,
            `Error processing ${REVENUE_CAT_NAME} job post ${id}`,
        );
...
export const revenueCatScrapper: NewCompanyScrapper = ({ companyId }) => {
    return {
        getListedJobPostsData: async () => {
            const response = await fetch(REVENUE_CAT_INITIAL_URL);
        ...
        scrapJobPost: async (jobPosts: ListedJobPostsData[]) => {
            ...
            for (let i = 0; i < jobPosts.length; i++) {
                try {
                    ...
                    const error = errorWithPrefix(
                        e,
                        `Error processing ${REVENUE_CAT_NAME}`,
                    );

...
const AFFIRM_NAME_INITIAL_URL =
    'https://boards-api.greenhouse.io/v1/boards/affirm/jobs?content=true';
...
const scrapJobPost = async ({
    ...
    try {
        ...
        const error = errorWithPrefix(
            e,
            `Error processing ${AFFIRM_NAME} job post ${id}`,
        );
...
export const affirmScrapper: NewCompanyScrapper = ({ companyId }) => {
    return {
        getListedJobPostsData: async () => {
            const response = await fetch(AFFIRM_NAME_INITIAL_URL);

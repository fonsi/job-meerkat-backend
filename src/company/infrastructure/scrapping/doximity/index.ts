...
export const DOXIMITY_NAME = 'doximity';
const INITIAL_URL =
    'https://boards-api.greenhouse.io/v1/boards/doximity/jobs?content=true';
...
const scrapJobPost = async ({
    ...
    try {
        ...
        const error = errorWithPrefix(
            e,
            `Error processing ${DOXIMITY_NAME} job post ${id}`,
        );
...
export const doximityScrapper: NewCompanyScrapper = ({ companyId }) => {
    return {
        ...
        scrapJobPost: async (jobPosts: ListedJobPostsData[]) => {
            ...
            for (let i = 0; i < jobPosts.length; i++) {
                try {
                    ...
                    const error = errorWithPrefix(
                        e,
                        `[Error processing ${DOXIMITY_NAME}]`,
                    );

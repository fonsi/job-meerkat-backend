...
export const BACKBLAZE_NAME = 'backblaze';
const INITIAL_URL =
    'https://boards-api.greenhouse.io/v1/boards/backblaze/jobs?content=true';
...
const scrapJobPost = async ({
    ...
    try {
        ...
        const error = errorWithPrefix(
            e,
            `Error processing ${BACKBLAZE_NAME} job post ${id}`,
        );
...
export const backblazeScrapper: NewCompanyScrapper = ({ companyId }) => {
    return {
        ...
        scrapJobPost: async (jobPosts: ListedJobPostsData[]) => {
            ...
            for (let i = 0; i < jobPosts.length; i++) {
                try {
                    ...
                    const error = errorWithPrefix(
                        e,
                        `[Error processing ${BACKBLAZE_NAME}]`,
                    );

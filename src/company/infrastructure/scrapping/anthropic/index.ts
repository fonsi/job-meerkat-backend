...
const ANTHROPIC_NAME_INITIAL_URL =
...
const scrapJobPost = async ({
    ...
    try {
        ...
        const error = errorWithPrefix(
            e,
            `Error processing ${ANTHROPIC_NAME} job post ${id}`,
        );
...
export const anthropicScrapper: NewCompanyScrapper = ({ companyId }) => {
    return {
        getListedJobPostsData: async () => {
            const response = await fetch(ANTHROPIC_NAME_INITIAL_URL);
            ...
            const jobPosts: JobPostsListItem[] = jobsData.jobs
                ...
                .filter(({ title }) => {
                    ...
                    return (
                        ...
                        !lower.includes('fellows program')
                    );
                });
        ...
        scrapJobPost: async (jobPosts: ListedJobPostsData[]) => {
            ...
            for (let i = 0; i < jobPosts.length; i++) {
                try {
                    ...
                    const error = errorWithPrefix(
                        e,
                        `Error processing ${ANTHROPIC_NAME}`,
                    );

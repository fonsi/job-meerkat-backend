...
export const VERCEL_NAME = 'vercel';
const VERCEL_NAME_INITIAL_URL =
...
const scrapJobPost = async ({
    ...
    try {
        ...
        const error = errorWithPrefix(
            e,
            `Error processing ${VERCEL_NAME} job post ${id}`,
        );
...
export const vercelScrapper: NewCompanyScrapper = ({ companyId }) => {
    return {
        getListedJobPostsData: async () => {
            const response = await fetch(VERCEL_NAME_INITIAL_URL);

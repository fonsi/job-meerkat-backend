...
export const CONFIANT_NAME = 'confiant';
const CONFIANT_INITIAL_URL =
    'https://boards-api.greenhouse.io/v1/boards/confiant/jobs';
...
const scrapJobPost = async ({
    ...
    try {
        const $ = await fromURL(
            `https://job-boards.greenhouse.io/embed/job_app?for=confiant&token=${id}`,
        );
        ...
        const error = errorWithPrefix(
            e,
            `Error processing ${CONFIANT_NAME} job post ${id}`,
        );
...
export const confiantScrapper: CompanyScrapperFn = async ({ companyId }) => {
    const response = await fetch(CONFIANT_INITIAL_URL);
    ...
    for (let i = 0; i < jobPosts.length; i++) {
        try {
            ...
            const error = errorWithPrefix(
                e,
                `[Error processing ${CONFIANT_NAME}]`,
            );

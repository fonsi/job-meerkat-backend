...
const POLAR_INITIAL_URL = 'https://api.ashbyhq.com/posting-api/job-board/polar';
...
const scrapJobPost = async ({
    ...
    try {
        ...
        const error = errorWithPrefix(
            e,
            `Error processing ${POLAR_NAME} job post ${id}`,
        );
...
export const polarScrapper: NewCompanyScrapper = ({ companyId }) => {
    return {
        getListedJobPostsData: async () => {
            const response = await fetch(POLAR_INITIAL_URL);

...
export const DEEPGRAM_NAME = 'deepgram';
const INITIAL_URL = 'https://api.ashbyhq.com/posting-api/job-board/deepgram';
...
const scrapJobPost = async ({
    ...
    try {
        ...
        const error = errorWithPrefix(
            e,
            `Error processing ${DEEPGRAM_NAME} job post ${id}`,
        );
...
export const deepgramScrapper: NewCompanyScrapper = ({ companyId }) => {
    return {
        getListedJobPostsData: async () => {
            ...
            jobsData.jobs.forEach((jobData) => {
                ...
                if (jobData.isListed && !title.includes('intern')) {
                    jobPosts.push({
                        id: jobData.id,
                        url: jobData.jobUrl,
                        title: jobData.title,
        ...
        scrapJobPost: async (jobPosts: ListedJobPostsData[]) => {
            ...
            for (let i = 0; i < jobPosts.length; i++) {
                try {
                    ...
                    const error = errorWithPrefix(
                        e,
                        `[Error processing ${DEEPGRAM_NAME}]`,
                    );

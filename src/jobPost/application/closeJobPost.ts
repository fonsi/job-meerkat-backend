...
import { JobPost, closeJobPost as closeJobPostEntity } from 'jobPost/domain/jobPost';
...
export const closeJobPost = async (command: CreateJobPostCommand) => {
    ...
    const jobPost = await jobPostRepository.getByOriginalIdAndCompanyId(originalId, companyId);

    if (!jobPost || jobPost.closedAt) {
        return;
    }

    const { id, companyId: entityCompanyId, closedAt } = closeJobPostEntity(jobPost);

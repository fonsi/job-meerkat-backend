...
const throwIfThreadsFailed = async (
    response: Response,
    prefix: string,
): Promise<void> => {
    if (response.ok) return;
    throw threadsResponseError(prefix, response.status, await response.text());
};
...
const createContainer = async (
    ...
    return withRetry(async () => {
        ...
        await throwIfThreadsFailed(response, 'Error creating thread:');
...
const publishContainer = async (id: string): Promise<string> => {
    ...
    return withRetry(async () => {
        ...
        await throwIfThreadsFailed(response, 'Error publishing thread:');
        const data = await response.json();
            ...
            try {
                ...
                if (!response.ok) {
                    const error = threadsResponseError(
                        'Error getting thread status:',
                        response.status,
                        await response.text(),
                    );
                    if (
                        !isRetriablePublishError(error) ||
                        retries >= STATUS_POLL_LIMIT
                    ) {
                ...
                if (retries >= STATUS_POLL_LIMIT) {
                    clearInterval(interval);
                    return reject(
                        new Error(
                            `Thread ${id} status ${status} not updated after ${STATUS_POLL_LIMIT} retries`,
                        ),
                    );
                }
            } catch (error) {
...
export const publishThread = async (posts: string[]): Promise<void> => {
    ...
    for (let i = 0; i < posts.length; i++) {
        const text = fitThreadsPost(posts[i]);
        if (!text) continue;
